...
function DocItem({ doc, onDelete }) {
  ...
  const handleDelete = () => {
    ...
    if (doc.status === "ready") {
      if (!window.confirm(`Remove "${doc.filename}" and its vector index? This cannot be undone.`)) return;
    }
    onDelete(doc.id);
  };
  ...
  return (
    <div className={`doc-item fade-in ${doc.status}`}>
      ...
      {doc.status !== "processing" && (
        <button className="doc-delete-btn" onClick={handleDelete} title="Remove document">
          <Trash2 size={12} />
        </button>
      )}
    </div>
